...
enum AffectedStat
{
	Health=0, Damage, BaseMaxHealth, MaxHealthMult, Armor, ShouldRunAway, UsefulAttackChanceMult, AggresiveAttackChanceMult,
...
func _RevertEffect(stats: Stats):
	match affected_stat:
		...
		AffectedStat.Armor:
			stats.SetArmor(_RevertOperation(stats.armor))
